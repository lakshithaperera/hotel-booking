(function () {
  ...
  function liveSearch() {
    ...
    targets.forEach(el => {
      ...
      const inputfield = el.querySelector('#property_search')
      let searchTerm = ''
...
(function () {
  ...
  var closeButton = $(".js-gallery-close-button");
...
(function () {
  var home_today = new Date();
  var home_tomorrow = new Date();
  home_tomorrow.setDate(home_tomorrow.getDate() + 1);

  if (document.querySelector('input').classList.contains('homapage-date-picker')) {
    var home_calendar_input = document.querySelector('#homapage-date-picker');

    home_calendar_input.value = fecha.format(home_today, 'YYYY-MM-DD') + ' - ' + fecha.format(home_tomorrow, 'YYYY-MM-DD');
    var datepicker = new HotelDatepicker(home_calendar_input, {
      clearButton: false,
      moveBothMonths: true,
      startOfWeek: 'monday',
      selectForward: true,
...
(function () {
  ...
  if (document.querySelector('input').classList.contains('booking_calander')) {
    var the_calendar = document.querySelector('#booking_calander');

    the_calendar.value = fecha.format(today, 'YYYY-MM-DD') + ' - ' + fecha.format(tomorrow, 'YYYY-MM-DD');

    var nights_count = document.querySelector('.nights-count');
    var the_calendar_input_2 = document.querySelector('.date-picker-input');

    // check availability fields
    var checkin_input = document.querySelector('#checkIn');
    var checkout_input = document.querySelector('#checkOut');
    var nationality_input = document.querySelector('#nationality_field');
    var flexi_input = document.querySelector('#flexi_field');



    var display_calendar = new HotelDatepicker(the_calendar, {
      showTopbar: false,
      moveBothMonths: true,
      startOfWeek: 'monday',
      selectForward: true,
      ...
      onSelectRange: function () {
        // console.log(display_calendar.getValue());
        the_calendar_input_2.value = display_calendar.getValue();
        nights_count.innerText = display_calendar.getNights();

        var split_date = the_calendar_input_2.value.split(" - ");
        var check_in_date = split_date[0];
        var check_out_date = split_date[1];
        checkin_input.value = fecha.format(fecha.parse(check_in_date, 'YYYY-MM-DD'), 'DD-MMM-YYYY');
        checkout_input.value = fecha.format(fecha.parse(check_out_date, 'YYYY-MM-DD'), 'DD-MMM-YYYY');

        console.log(fecha.format(fecha.parse(check_in_date, 'YYYY-MM-DD'), 'DD-MMM-YYYY'));
      }
    });
    the_calendar_input_2.value = display_calendar.getValue();
    nights_count.innerText = display_calendar.getNights();
    ...
    console.log(the_calendar_input_2.value.split(" - "));
